const DEFAULTS = {
  baseUrl: '', apiKey: '', batchSize: 12, concurrency: 1, gapMs: 900, cycleDelaySec: 20, enabled: false,
};
const FIELDS = ['baseUrl', 'apiKey', 'batchSize', 'concurrency', 'gapMs', 'cycleDelaySec'];
...
document.getElementById('save').addEventListener('click', () => {
  ...
  // Ceilings, not suggestions: this decides how hard a real browser hits the retailer the client
  // buys from. Tighter than the Pokemon Center bridge on every axis, deliberately.
  out.concurrency = Math.min(4, out.concurrency);
  out.batchSize = Math.min(60, out.batchSize);
  out.gapMs = Math.max(400, Math.min(10000, out.gapMs));
...
document.getElementById('refresh').addEventListener('click', () => {
  chrome.runtime.sendMessage({ type: 'amz-refresh' });
});
