...
describe('curl fetch strategy', () => {
  ...
  test('a bad hostname resolves to null rather than throwing', async () => {
    const res = await curlGet('https://this-host-does-not-exist.invalid/x', { timeoutMs: 5000 });
    assert.strictEqual(res, null, 'a fetch failure must fall through to the next strategy, not crash the poll');
  });
