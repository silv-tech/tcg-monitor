...
describe('E2E Simulation: 50 SKUs go live', () => {
  ...
  it('builds valid embeds for all event types', () => {
    ...
    for (const event of eventTypes) {
      ...
      assert.ok(json.fields && json.fields.length > 0, `Embed should have fields for ${event.type}`);
    }
  });
  ...
  it('embed price change shows old/new values', () => {
    ...
    const embed = buildEmbed(event);
    const json = embed.toJSON();
    // Price info is now in the Price field
    const priceField = json.fields.find(f => f.name === 'Price');
    assert.ok(priceField, 'Should have Price field');
    assert.ok(priceField.value.includes('199.99'), 'Should show old price');
    assert.ok(priceField.value.includes('149.99'), 'Should show new price');
  });
